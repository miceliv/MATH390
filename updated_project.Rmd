...
d %<>%
  ...
  mutate(num_floors_in_building = num_floors_in_building)
```
...
real_d = subset(d, (!is.na(d[,15])))  ## sale price
fake_d = subset(d, (is.na(d[,15])))
```
...
X[,17][X[, 17] < 1000] = NA ## number 17 is total taxes
...
Ximp = missForest(data.frame(X), ntree = 500, mtry = 22, sampsize = rep(172, ncol(X)))$ximp
```


```{r}
Xnew = data.frame(cbind(Ximp, M))
linear_mod_impute_and_missing_dummies = lm(sale_price ~ ., data = Xnew)
...
```{r}
summary(lm(price ~ ., data = Data)) ## simple linear model
```


```{r}
summary(lm(price ~ .*., data = Data)) ## with single interactions
```


```{r}
mod1 = lm(Ytrain ~ ., data = Tr)
summary(mod1)
```
