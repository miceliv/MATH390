...
Ximp = missForest(data.frame(X), ntree = 500, mtry = 21, sampsize = rep(172, ncol(X)))$ximp
```
...
Data1 %<>%
  filter(!is.na(d.sale_price))


Y = Data1$d.sale_price
...
```{r}
linear = lm(d.sale_price ~ ., data = Data1)## simple linear model
summary(linear)
```
