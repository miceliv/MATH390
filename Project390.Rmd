...
test_indices = setdiff(1:nrow(real_d), train_indices)
training_data = real_d[train_indices, ]
testing_data = real_d[test_indices, ]
...
#Some of these missing indicators are collinear.
...
#Some featuress did not have missingness so let's remove them:
...
# Many observations are missing y values. We 
```{r}
Data = Xnew
...
```{r}
yhat_test_lin = predict(linear, Xtest)

e_lin = yhat_test_lin - Ytest

#RMSE
sqrt(sum(e_lin^2)/length(e_lin))
```
...
#Running this chunk gives the optimal hyperparameters used in the RF model.
